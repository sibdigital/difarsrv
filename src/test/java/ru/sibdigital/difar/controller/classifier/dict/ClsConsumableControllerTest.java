...
public class ClsConsumableControllerTest extends ClsBaseControllerTest {
    ...
    public void requestsTest() throws Exception {
        ...
        entity.setName(DEFAULT_NAME)
                ...
                .setClsConsumableKindByIdConsumableKind(consumableKindRepository.findById(DEFAULT_ID).get())
                .setClsUnitByIdUnit(unitRepository.findById(DEFAULT_ID).get());

        String json = ow.writeValueAsString(entity);

        MvcResult result = this.mockMvc.perform(post("/consumable/create").contentType("application/json").content(json))
        ...
        this.mockMvc.perform(get("/consumable/" + id))
                .andExpect(status().isOk())
                .andExpect(content().contentType("application/json"))
                .andExpect(jsonPath("$.name").value(DEFAULT_NAME))
                .andDo(print());
        ...
        this.mockMvc.perform(put("/consumable/update").contentType("application/json").content(json))
                .andExpect(status().isOk())
                .andExpect(content().contentType("application/json"))
        ...
        this.mockMvc.perform(delete("/consumable/" + id))
                .andExpect(status().isOk())

...
public class ClsUnitRatioControllerTest extends ClsBaseControllerTest {
    ...
    public void requestsTest() throws Exception {
        ...
        entity.setName(DEFAULT_NAME)
                ...
                .setClsUnitByIdUnitFrom(unitRepository.findById(DEFAULT_ID).get())
                .setClsUnitByIdUnitTo(unitRepository.findById(DEFAULT_ID).get());

        String json = ow.writeValueAsString(entity);

        MvcResult result = this.mockMvc.perform(post("/unit-ratio/create").contentType("application/json").content(json))
        ...
        this.mockMvc.perform(get("/unit-ratio/" + id))
                .andExpect(status().isOk())
                .andExpect(content().contentType("application/json"))
                .andExpect(jsonPath("$.name").value(DEFAULT_NAME))
                .andDo(print());
        ...
        this.mockMvc.perform(put("/unit-ratio/update").contentType("application/json").content(json))
                .andExpect(status().isOk())
                .andExpect(content().contentType("application/json"))
        ...
        this.mockMvc.perform(delete("/unit-ratio/" + id))
                .andExpect(status().isOk())
